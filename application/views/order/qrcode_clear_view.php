...
<div class="mt-4 text-center" id="send_section">
    <?php if($order->status_order=='4'){;?>
        ...
        <?php }elseif($order->status_order=='9'){;?>
            ...
            <?php }else{;?>
                 <button class="btn btn-success txtbtn" id="confirm">ยืนยันการเคลียร์เงิน</button>
         <?php   };?>
</div>
    </div>
</div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body logouttxt text-center">
                <div class="mt-4 mb-4">
                   ยืนยันการเคลียร์เงินสำเร็จ
                </div>
                <div>
                    <button class="btn btn-success txtbtn" type="button" data-dismiss="modal">ตกลง</button>
                </div>
            </div>
        </div>
    </div>
    ...
    $('#confirm').click(function(){
        ...
        function getstatus(){
            ...
            success: function(response) {
                ...
                if (response) {
                    console.log(response);
                    $('#send_section').html('<button class="btn btn-secondary txtbtn" disabled>'+response.status_label+'</button>');
                    // alert(response.status_label);
                    // $('#send_success').modal('show');
                    // $('#result').html(html);
                } else {
